...
<div class="login" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	<form name="lostpasswordform" id="lostpasswordform<?php $template->the_instance(); ?>" action="<?php $template->the_action_url( 'lostpassword' ); ?>" method="post">
		<div class="inputPair inputSet set-text">
			...
			<input type="text" name="user_login" id="user_login<?php $template->the_instance(); ?>" required="required" class="text required" value="<?php $template->the_posted_value( 'user_login' ); ?>" size="20" />
		</div>
<?php
do_action( 'lostpassword_form' ); // Wordpress hook
do_action_ref_array( 'tml_lostpassword_form', array( &$template ) ); // TML hook
?>
		<div class="inputSet inputPair set-submit">
